...
A classic maze ball-rolling game based on the Android platform. This project fixes, refactors, and modernizes the decompiled code from [teeter_htc_decomp](https://github.com/ceclandumitru/teeter_htc_decomp), making the originally non-compilable decompiled output buildable and runnable on current Android devices. It recreates the classic Teeter maze ball game that was pre-installed on HTC phones. Players tilt their device (using the accelerometer) to control the ball through the maze, avoid traps, and reach the finish.

## Project Origin

The code of this project originates from the decompiled reconstruction of the HTC built-in game `com.htc.android.teeter`:

- **Original decompiled project**: [ceclandumitru/teeter_htc_decomp](https://github.com/ceclandumitru/teeter_htc_decomp)
...
1. After launching the game, a splash animation plays, then the game main screen appears
2. Tilt the device to control the ball rolling through the maze
3. The ball must avoid holes (traps) and reach the finish to complete the level
4. Each level records the completion time and number of attempts
5. Game progress is automatically saved and can be resumed on the next launch
